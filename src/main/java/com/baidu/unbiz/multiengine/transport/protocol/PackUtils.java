...
public class PackUtils {
    ...
    public static List<byte[]> buildPackData(long seqId, byte[] buf, int capacity) {
        if (capacity < MsgHead.SIZE) {
            throw new MultiEngineException("packSize is too small");
        }
        List<byte[]> dataList = new ArrayList<byte[]>();

        int remainSize = countRemainSize(buf.length, capacity - MsgHead.SIZE);
        int sumLength = buf.length;
        int index = 0;
        do {
            int packLen = Math.min(sumLength - index + MsgHead.SIZE, capacity);
            byte[] pack = new byte[packLen];
            int bodyLen = packLen - MsgHead.SIZE;
            System.arraycopy(buf, index, pack, MsgHead.SIZE, bodyLen);

            MsgHead head = MsgHead.create(seqId);
            head.setSumLen(sumLength);
            head.setRemainLen(remainSize);
            head.setBodyLen(bodyLen);
            System.arraycopy(head.toBytes(), 0, pack, 0, MsgHead.SIZE);

            index += packLen - MsgHead.SIZE;
            remainSize = countRemainSize(buf.length - index, capacity - MsgHead.SIZE);
            dataList.add(pack);
